...
// 1. Definimos una lista de clientes que llegan al mismo tiempo
const clientes = [
  { nombre: "Ana", pedido: "Pizza" }, // 3 segundos
  { nombre: "Luis", pedido: "Hamburguesa" }, // 2 segundos
  { nombre: "Maria", pedido: "Ensalada" }, // 1 segundo
];
...
Promise.race(promesasPedidos)
  .then((primerPedidoListo) => {
    ...
    console.log(`\n*****************************************`);
    console.log(`¡ATENCIÓN MESEROS! El primer pedido listo es: ${primerPedidoListo}`);
    console.log(`*****************************************`);
  })
  .catch((error) => {
...
// 4. Usamos Promise.all para esperar a que TODOS los pedidos estén listos.
...
Promise.all(promesasPedidos)
  ...
  .catch((error) => {
    ...
    console.error("\n Ocurrió un error en la cocina con uno de los pedidos:", error);
